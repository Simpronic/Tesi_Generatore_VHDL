...
def compute_trivially_shared_ngrams(methods: list, language: str, cache_dir: str, ngrams_range: range = range(1, 5)) -> dict:
    ...
    #     print(
    #         f'Loading trivially shared n-grams from file {ngrams_filename} ...')
    ...
    # txt_filepath_ngrams = os.path.join(
    #     cache_dir, f'{language}_trivially_shared_ngrams.txt')
...
def compute_crystal_bleu(references: list, candidates: list, trivial_ngrams: dict, language: str, weights: tuple = (1./4., 1./4., 1./4., 1./4.)) -> float:
    ...
    for ref, cand, in zip(references, candidates):
        ref_tokens = ' '.join(
            [t[1] for t in lexer.get_tokens(ref) if t[1].strip()]).split()
        cand_tokens = ' '.join(
            [t[1] for t in lexer.get_tokens(cand) if t[1].strip()]).split()
        score = corpus_bleu([[ref_tokens]], [cand_tokens],
                            weights=weights, ignoring=trivial_ngrams,
                            smoothing_function=smoothing)
        scores.append(round(score, 2))
    return scores


# dataset_files = ["train_data.json", "eval_data.json", "test_data.json"]
# list_methods = []
# for file in dataset_files:
#     examples = load_dataset('json', data_files=file, split="train")
#     for code in examples['output']:
#         list_methods.append(code)

# print(len(list_methods))
# language = 'python'
# trivial_ngrams = compute_trivially_shared_ngrams(list_methods, language, "crystal_cache")

# references, candidates = [], []
# scores = compute_crystal_bleu(references, candidates, trivial_ngrams, language)
